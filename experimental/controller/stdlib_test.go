...
// unescapeOneLevel strips one '$' from '$${}' patterns in a string,
// simulating what evalString does when evaluating a template that contains
// deferred expressions. Used by unit tests that extract nested Graphs from
// Kind templates and compile them standalone.
func unescapeOneLevel(s string) string {
	return strings.ReplaceAll(s, "$${", "${")
}

// unescapeNodeSlice applies unescapeOneLevel to all string values in a
// parsed node list. This simulates L1 evaluation stripping one escape level.
	...
	isWinner := func(schema map[string]any, peers []any) bool {
		...
		for i, p := range peers {
			...
			identities[i] = map[string]any{
				...
				"target":   resourceIdentity(pm),
			}
		}

		myTarget := resourceIdentity(schema)

		// Build candidates list (simulates the forEach definition).
		var candidates []any
		for _, id := range identities {
			if id.(map[string]any)["target"] == myTarget {
