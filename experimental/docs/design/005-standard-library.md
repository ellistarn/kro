...
Stdlib:  Kind      — define a new Kubernetes Kind with schema and per-instance behavior
...
A Decorator attaches resources to instances of an existing Kind. It does not define a new Kind — it
watches an existing one and creates resources per instance.

Two modes:

- **WatchKind** — no `metadata.name` in the watch spec (or empty string). Iterates all matching
  instances. The scope variables are `item` (per-instance) and `items` (the collection).
- **Watch** — has a non-empty `metadata.name`. Targets a single resource. The scope variable is
  `item`.
...
### Implicit forEach Rule

Nodes in a WatchKind Decorator without an explicit `forEach` get an implicit
`forEach: {item: "${items}"}`. Nodes with an explicit `forEach` pass through unchanged. This
unifies per-item and collection-level semantics. A node that operates per-instance writes
`${item.metadata.name}` — the forEach is added automatically. A node that aggregates across the
collection provides its own forEach and is left alone.

For the Watch case (single resource), the scope variable is `item` and no forEach is added.

### Deletion

Decorators do not guarantee ordered cleanup. When the watched Kind is deleted (CRD removed), all
instances vanish simultaneously. Resources created by the Decorator are owned by its compiled Graph
and are garbage-collected through normal Kubernetes ownership. The Decorator does not get a signal
to clean up in a specific order before its targets disappear.
...
- `kinds.experimental.kro.run` — created by `kind.yaml` (a Graph node)
...
Three resources are embedded in `experimental/stdlib/` and applied after the controller starts:

| Resource         | `kind:` used | Purpose |
|------------------|--------------|---------|
| `kind.yaml`      | Graph        | Creates Kind CRD, watches/processes all Kinds |
| `decorator.yaml` | Kind         | Defines Decorator type |
| `singleton.yaml` | Kind         | Defines Singleton type, creates a Decorator for resolution |

### Bootstrap Sequence

1. Bootstrap: apply Graph and GraphRevision CRDs via SSA, wait for Established
2. Controller starts
3. Stdlib Runnable applies all 3 resources via SSA with retry
4. `kind.yaml` (a Graph) succeeds immediately — Graph CRD exists
5. Other resources fail (CRD not found) — retried on 2-second interval
...
targets — the Decorator's Graph is cleaned up through normal ownership, but the Decorator does not
get ordered cleanup semantics. Resources created by the Decorator are owned by its compiled Graph
and are garbage-collected. A finalizer on Kind to block deletion until confirmed is not yet
implemented.
