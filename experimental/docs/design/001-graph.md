...
Expands the template once per item in a list. The list is a CEL expression referencing a WatchKind
...
Two failure domains require different responses: a malformed spec is a developer problem (fix the
Graph definition); a runtime failure is an operator problem (fix the cluster state, permissions, or
infrastructure). The condition structure reflects this split.
