...
(see change detection below). If the propagate-hash changed — or no previous hash exists —
...
node runs the full pipeline (steps 1-8). The drift timer bypasses the evaluation-hash check (step 4) and
the apply-hash check (step 6) — evaluate the template and apply unconditionally, because
server-side defaulters and mutating webhooks can change fields without changing the desired state
hash. The propagate-hash (step 8) still applies after drift correction — if the corrected output
matches the previous output, downstream nodes are not triggered. SSA is idempotent; the apply corrects drift as a side effect.
...
| propagate-hash | output field paths downstream references | propagation stops | trigger downstream nodes |
...
The propagate-hash still applies after drift correction; if the corrected output matches the
previous output, downstream nodes are not triggered.

Input-hash and propagate-hash share the same path extraction. At graph compilation, the controller
...
8. **Propagation check** — hash the output paths downstream expressions reference plus propagateWhen
   state, compare against in-memory propagate-hash from previous evaluation. Differs → node
