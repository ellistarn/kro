...
A content hash on each revision identifies semantically identical output across generations. Because
the revision contains exactly what the developer authored (no injected metadata), the content hash
reflects the full revision content with no carve-outs. Every successful generation gets its own
revision object regardless — the content hash enables the controller to detect when a spec change
produced no actual resource changes and skip the rollforward.
