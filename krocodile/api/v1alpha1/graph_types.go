...
//
// +kubebuilder:validation:XValidation:rule="[has(self.template), has(self.ref), has(self.watch), has(self.def)].exists_one(x, x)",message="exactly one of template, ref, watch, def must be set"
type Node struct {
