...
func validateGraph(g *expv1alpha1.Graph) error {
	...
	seen := make(map[string]struct{}, len(g.Spec.Nodes))
...
// a given node kind. Today: ref and watch nodes can't carry forEach
// because they don't render templates — they read existing state. A user
// who wants per-element behavior should iterate the published list with
// CEL's all/map/exists in a downstream node.
...
func validateNodeShape(n *expv1alpha1.Node) error {
	...
	if set != 1 {
		return fmt.Errorf("exactly one of template/ref/watch/def must be set (got %d)", set)
	}
	return nil
}
